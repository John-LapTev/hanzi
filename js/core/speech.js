...
export const VOICE_COST = {
  usd: 1.97,           // синтез речи через MediaFlow, суммарно за всё время
  rate: 84.54,         // ₽ за доллар, официальный курс ЦБ РФ
  updated: '16.08.2026',
  clips: 961,          // записей слов, примеров и знаков
...
export const speech = {
  ...
  async init() {
    try {
      const response = await fetch(AUDIO_INDEX_URL);
      if (response.ok) {
    ...
    try {
      const response = await fetch(SYLLABLE_AUDIO_URL);
      if (response.ok) {
        const index = await response.json();
