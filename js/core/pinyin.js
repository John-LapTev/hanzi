...
/** Обратная таблица: символ с диакритикой → базовая буква и номер тона.
    Заглавные тоже: имена собственные пишутся с большой буквы (Éluósī «Россия»), и без них
    тренировка тонов считала É знаком без тона — то есть нейтральным (жалоба владельца 25.08.2026). */
const MARK_TO_PLAIN = (() => {
  const map = new Map();
  Object.keys(TONE_ROWS).forEach((letter) => {
    TONE_ROWS[letter].split('').forEach((marked, index) => {
      map.set(marked, { letter, tone: index + 1 });
      map.set(marked.toUpperCase(), { letter, tone: index + 1 });
    });
  });
  return map;
})();
