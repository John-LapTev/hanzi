...
/** Верные, ошибки и чистый счёт слова. Ошибка тона — тоже ошибка, но своя: слог узнан. */
export function wordScore(word) {
  const record = state.srs.get(word.id);
  if (!record) return { right: 0, wrong: 0, score: 0, seen: 0 };
  const wrong = (record.errors || 0) + (record.toneErrors || 0);
...
/** Размер знака в плитке: слово из четырёх иероглифов иначе переносится на две строки. */
const HANZI_SIZES = { 1: 32, 2: 28, 3: 21, 4: 17 };
const hanziSize = (hanzi) => HANZI_SIZES[Math.min(Array.from(hanzi).length, 4)] || 15;
...
function wordTile(word, counts) {
  ...
  return el('button', {
    ...
    title: `${word.hanzi} — ${word.translation}\nверно ${counts.right}, ошибок ${counts.wrong}, `
      + `счёт ${counts.score} · ${rank.title}`,
    'aria-label': `${word.hanzi}: верно ${counts.right}, ошибок ${counts.wrong}`,
    onclick: () => { if (speech.available) speech.speak(word.hanzi); },
  }, [
    ...
    el('span', { class: 'hanzi mastery-hanzi', text: word.hanzi,
      style: `font-size:${hanziSize(word.hanzi)}px` }),
    el('span', { class: 'mastery-pinyin', text: word.pinyin }),
...
export function renderMastery() {
  const words = studiedWords().map((word) => Object.assign({ word }, wordScore(word)))
    .sort((first, second) => second.score - first.score
      ...
      || first.word.hanzi.localeCompare(second.word.hanzi));
