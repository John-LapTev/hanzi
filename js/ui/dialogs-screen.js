...
export function renderDialogList() {
  fill('dialog-list', DIALOGS.map((dialog) => {
    ...
    return el('button', {
      class: 'lesson-row', type: 'button', onclick: () => openDialog(dialog.id, { screen: 'grammar' }),
    }, [
...
function openDialog(dialogId, from) {
  if (from) state.cameFrom.dialog = from;
  ...
  syncBackButton('dialog-back', 'dialog', '← К разговорам', 'grammar');
  renderDialog();
}
...
function renderDialog(feedback) {
  ...
  const input = el('input', {
    ...
    // Черновик переживает перерисовку: подсказка больше не стирает набранное
    value: state.dialogDraft || '',
  });
  state.dialogDraft = '';
  ...
  const answered = Boolean(feedback && feedback.status === 'correct');
  const task = [
    el('p', { class: 'card-question', text: hard ? 'Твой ответ' : line.prompt }),
    input,
    el('div', { class: 'row', style: 'margin-top:12px' }, [
      // После верного ответа кнопка отключается: раньше она оставалась живой, смотрела уже
      // на пустое поле и стирала «Верно» вместе с кнопкой «Дальше» (аудит 03.09.2026)
      el('button', { class: 'btn btn-small', type: 'button', disabled: answered,
        onclick: () => submitDialogAnswer(input.value) }, 'Проверить'),
      hard ? null : el('button', {
        class: 'btn btn-quiet btn-small', type: 'button',
        // Набранное сохраняем: перерисовка ради подсказки стирала половину написанной фразы
        onclick: () => { state.dialogDraft = input.value; state.dialogHint = !state.dialogHint; renderDialog(feedback); },
      }, state.dialogHint ? 'Скрыть подсказку' : 'Подсказка'),
      el('button', {
        class: 'btn btn-quiet btn-small', type: 'button', disabled: answered,
