...
export function renderLessonList() {
  fill('lesson-list', GRAMMAR_LESSONS.map((lesson, index) => {
    ...
    return el('button', {
      class: 'lesson-row', type: 'button', onclick: () => openLesson(lesson.id),
    }, [
...
export function openLesson(lessonId) {
  state.lesson = GRAMMAR_LESSONS.find((lesson) => lesson.id === lessonId);
  state.drillIndex = 0;
  state.drillChunks = [];
...
function renderDrill(feedback) {
  ...
  const children = [
    ...
    slot,
    renderDrillBank(drill, chosen, feedback),
  ];
