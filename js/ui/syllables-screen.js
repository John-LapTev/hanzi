...
const SYLLABLE_STREAK_TO_LEARN = 3;
...
function syllableRecord(syllable) {
...
function runsSinceSeen(syllable) {
 ...
 */
function pickDrillQueue(pool, size, ordered) {
...
function syllableStatus(syllable) {
  const record = state.syllableProgress.get(syllable);
...
async function markSyllableShown(syllables) {
...
async function noteSyllableAnswer(syllable, correct) {
  const record = syllableRecord(syllable);
  if (correct) {
    record.right += 1;
    record.streak += 1;
    ...
    const children = [
      el('div', { class: 'card center' }, [
        ...
        el('button', { class: 'btn btn-quiet btn-wide', type: 'button',
          onclick: () => showScreen('pinyin') }, 'К слогам'),
      ]),
    ];
...
export function handleSyllableKey(event) {
  ...
  if (event.key === 'Escape') { showScreen('pinyin'); return; }
  ...
  if (event.key === '0') { checkSyllableAnswer(run.queue[run.position], true); return; }
