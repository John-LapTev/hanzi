...
import { MODES, TONE_LABELS, answerJudge, dueWords, finishExam, isToneReady, isWordAvailable, matchesTopic, nextQuestion, recordAnswer, renderClozeQuestion, renderJudgeQuestion, renderSentenceQuestion, session, shuffle, startExam, startSession, troubleWords } from '../core/modes.js';
...
function renderChoiceQuestion(question, feedback) {
  ...
  const options = question.options.map((option, index) => {
    ...
    return el('button', {
      ...
      el('span', { class: question.optionField === 'hanzi' ? 'hanzi' : '', text: label }),
    ]);
  });
...
function renderFlipQuestion(question) {
  ...
  const card = el('div', { class: 'train-card' }, session.flipped ? back : front);
...
async function answerFlashcard(quality) {
  ...
  await recordAnswer(session.question.word, verdict);
  advanceQuestion();
}
