...
function renderModeGrid() {
  const grid = fill('mode-grid', MODES.map((mode) => {
    ...
    return el('button', {
      ...
      disabled: blocked,
      title: blocked ? 'Нужен китайский голос в системе' : mode.description,
      onclick: () => { state.mode = mode.id; renderModeGrid(); },
    }, [el('span', { class: 'mode-icon hanzi', text: mode.icon }), el('span', { text: mode.title })]);
  }));
  ...
  document.getElementById('mode-description').textContent = missingVoice
    ? `${active.description} Режимы со звуком выключены: в системе нет китайского голоса.`
    : active.description;
  return grid;
}
