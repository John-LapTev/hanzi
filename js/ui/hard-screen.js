...
/* ——— Раздел «Трудные слова» ———
   Свой список, свой прогресс, пять режимов подряд. Ни к дню программы, ни к дневной
   норме повторений он не привязан: сюда заходят, когда хочется добить слова, которые
   всё время угадываются.                                                              */

export function renderHardScreen() {
  const words = hardWords();
  const body = document.getElementById('hard-body');
  document.getElementById('hard-back').textContent =
    state.hardReturn === 'teacher-day' ? '← К дню' : '← На главную';
  ...
  const children = [
    el('div', { class: 'card' }, [
      ...
      el('p', { class: 'faint', style: 'margin:0', text: progress.complete
        ? 'Все слова прошли все пять режимов. Можно начать круг заново или снять метки с тех, что уже держатся в голове.'
        : `Полностью отработано ${progress.fullyDone} из ${inWork.length} `
          + `${plural(inWork.length, 'слова', 'слов', 'слов')}: слово считается пройденным, `
          + `когда одолело все пять режимов (${progress.done} из ${progress.total} проходов).` }),
    ]),
  ];
  ...
  children.push(el('div', { class: 'hard-modes' }, HARD_MODES.map((mode, index) => {
    ...
    return el('button', {
      ...
      el('span', { class: 'hard-mode-icon hanzi', text: mode.icon }),
  ...
  words.forEach((word) => {
    ...
    children.push(el('div', { class: `word-row${untouchedWord ? ' is-locked' : ''}` }, [
      ...
      speech.available ? el('button', {
        class: 'hard-round', type: 'button', 'aria-label': `Озвучить ${word.hanzi}`,
        onclick: () => speech.speak(word.hanzi),
      }, uiIcon('sound', 16)) : null,
      el('button', {
        class: 'hard-round hard-drop', type: 'button', 'aria-label': `Снять метку с ${word.hanzi}`,
...
/** Карточка раздела: показывается на главной и в дне программы, когда есть что повторять. */
export function hardCard(fromScreen) {
  ...
  return el('div', { class: 'card' }, [
    ...
    el('p', { class: 'faint', text: progress.complete
      ...
      : 'Те, что ты пометил кружком. Отдельный прогон, на программу дня он не влияет.' }),
    el('button', {
