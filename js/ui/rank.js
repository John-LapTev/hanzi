...
/* ——— Ранг ученика: 段位, ступени Китайской ассоциации ушу ———
   ...
   Ступеней девять: три зверя (орёл → тигр → дракон), внутри каждого три металла
   (лазурь → серебро → золото). Названия настоящие, они же работают словарём:
   青 «лазурный», 银 «серебряный», 金 «золотой», 鹰 «орёл», 虎 «тигр», 龙 «дракон».
   Персонаж — 侠客 (сякэ), странствующий воин: халат с запáхом, прямой меч цзянь.  */
...
const XP_PER_LESSON = 10;        // пройденный урок или сценка
const XP_PER_STROKE = 2;         // разобранный знак в прописях
const XP_PER_PINYIN_LESSON = 5;  // прочитанный урок пиньиня
...
const PLAYER_RANKS = [
  { level: 1, xp: 0, hanzi: '青鹰', pinyin: 'qīng yīng', name: 'Лазурный орёл', beast: 'eagle', metal: 'lazur' },
  { level: 2, xp: 60, hanzi: '银鹰', pinyin: 'yín yīng', name: 'Серебряный орёл', beast: 'eagle', metal: 'serebro' },
  { level: 3, xp: 160, hanzi: '金鹰', pinyin: 'jīn yīng', name: 'Золотой орёл', beast: 'eagle', metal: 'zoloto' },
  { level: 4, xp: 320, hanzi: '青虎', pinyin: 'qīng hǔ', name: 'Лазурный тигр', beast: 'tiger', metal: 'lazur' },
  { level: 5, xp: 560, hanzi: '银虎', pinyin: 'yín hǔ', name: 'Серебряный тигр', beast: 'tiger', metal: 'serebro' },
  { level: 6, xp: 900, hanzi: '金虎', pinyin: 'jīn hǔ', name: 'Золотой тигр', beast: 'tiger', metal: 'zoloto' },
  { level: 7, xp: 1400, hanzi: '青龙', pinyin: 'qīng lóng', name: 'Лазурный дракон', beast: 'dragon', metal: 'lazur' },
  { level: 8, xp: 2100, hanzi: '银龙', pinyin: 'yín lóng', name: 'Серебряный дракон', beast: 'dragon', metal: 'serebro' },
  { level: 9, xp: 3000, hanzi: '金龙', pinyin: 'jīn lóng', name: 'Золотой дракон', beast: 'dragon', metal: 'zoloto' },
];
...
function playerExperience() {
  ...
  const lessons = Array.from(state.grammarProgress.values()).filter((item) => item.done).length;
  return correct * XP_PER_CORRECT
    + learned * XP_PER_LEARNED
    + exams * XP_PER_EXAM
    + lessons * XP_PER_LESSON
    + state.strokesSeen.size * XP_PER_STROKE
    + state.pinyinSeen.size * XP_PER_PINYIN_LESSON;
}
...
/* ——— Рисунки ———
   Четыре штуки: воин, орёл, тигр, дракон. Металл задаётся параметром, поэтому
...
/** Тигр: голова анфас, на лбу 王 — без него китайского тигра не узнать. Середина пути. */
function tigerEmblem(metal, size) {
  ...
  // 王 на лбу
  svg.append(svgEl('path', Object.assign({ d: 'M48 34h24M48 46h24M48 58h24M60 34v24' }, ink)));
  // полосы по бокам
...
/** Дракон: китайский — длинное тело кольцом, рога и усы, крыльев нет. Вершина. */
...
/** Странствующий воин: халат с запáхом, кушак, прямой меч цзянь. Ни катаны, ни маски —
    это признаки японского воина, а здесь другая культура. Облик растёт со ступенью. */
function warriorFigure(rank, size) {
  ...
  // плащ у дракона — рисуем первым, он за спиной
  if (tier === 'dragon') {
    svg.append(svgEl('path', Object.assign({
      d: 'M74 40 C104 52 108 84 96 106 L74 96 Z', opacity: '.55' }, cloth)));
  }

  // меч цзянь за спиной (у дракона он в руке — ниже)
  if (tier !== 'dragon') {
  ...
  // халат до голеней, запáх направо
  svg.append(svgEl('path', Object.assign({
  ...
  if (tier === 'tiger') {
    // соломенная шляпа доули надета, наручи
  ...
  if (tier === 'eagle') {
    // соломенная шляпа висит за спиной на шнурке — путь только начинается
...
export function renderPlayerBadge() {
  ...
  const button = el('button', {
    ...
    title: `${rank.hanzi} ${rank.pinyin} — ${rank.name}, опыт ${state_.xp}`,
    'aria-label': `Ступень ${rank.level}, ${rank.name}`,
    onclick: () => { state.progressTab = 'awards'; showScreen('progress'); },
  }, [
    el('span', { class: 'badge-beast hanzi', style: `color:${RANK_METALS[rank.metal].soft}`,
      text: rank.hanzi.slice(1) }),
  ...
  return el('div', { class: `card rank-card${top ? ' is-top' : ''}` }, [
    el('div', { class: 'rank-row' }, [
      ...
      el('div', { class: 'rank-titles' }, [
        el('div', { class: 'rank-hanzi hanzi', text: rank.hanzi }),
        el('div', { class: 'rank-pinyin', text: rank.pinyin }),
    ...
    el('p', { class: 'faint', text: progress.next
      ? `До ступени «${progress.next.hanzi} ${progress.next.name.toLowerCase()}» — ${progress.toNext} опыта. `
        + 'Опыт дают верные ответы, выученные слова, экзамены, уроки и разобранные знаки.'
      : 'Выше некуда: 金龙, золотой дракон — девятая ступень.' }),
  ]);
}
