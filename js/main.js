import { AUDIO_CACHE, DEFAULT_SESSION_LIMIT, DEFAULT_SPEECH_RATE, STORE_EXAMS, STORE_GRAMMAR, STORE_HARD, STORE_SRS, STORE_STATS, STORE_SYLLABLES, STORE_WORDS, USER_LEVEL } from './core/constants.js';
...
import { switchGrammarTab, switchPinyinTab } from './ui/tabs.js';
...
import { restartTeacherTask } from './ui/teacher-tasks.js';
import { restartSyllableDrill } from './ui/syllables-screen.js';
import { exitHardDrill, handleHardKey, restartHardDrill } from './ui/hard-drill.js';

/* ═══════════════════ Запуск ═══════════════════ */

export async function loadEverything() {
  const [words, srsRecords, statsRecords, examRecords, grammarRecords, syllableRecords, hardRecords] =
    await Promise.all([
...
function bindEvents() {
  ...
  // Раздел открывают и с главной, и из дня программы — кнопка возвращает туда, откуда пришли
  ...
  document.getElementById('syllable-restart').addEventListener('click', restartSyllableDrill);
