/* Service Worker приложения hanzi — офлайн-кеш.
 *
 ...
 */
const CACHE = 'hanzi-v79';          // имя проверяет tools/publish.sh — не переименовывать
const SHELL = `${CACHE}-shell`;
const AUDIO = 'hanzi-audio';
...
const ASSETS = [
  ...
  './data/strokes.json',      // прописи: порядок черт, ~690 КБ
  './audio/index.json',       // карта «фраза → запись произношения»
  './audio/syllables/index.json',   // карта «слог таблицы → запись»

  // модули приложения — без них офлайн не запустится
  ...
  './js/data/dialogs.js',
  './js/data/grammar.js',
  './js/data/pinyin-lessons.js',
  './js/data/stroke-rules.js',
  ...
  './js/ui/home.js',
  './js/ui/icons.js',
  './js/ui/pinyin-screen.js',
  './js/ui/rank.js',
  './js/ui/screens.js',
  './js/ui/sentence.js',
  './js/ui/strokes-screen.js',
  './js/ui/syllables-screen.js',
  './js/ui/teacher-course.js',
  './js/ui/teacher-strokes.js',
...
self.addEventListener('fetch', (event) => {
  ...
  const isCode = url.pathname.endsWith('.js') || url.pathname.endsWith('.html')
